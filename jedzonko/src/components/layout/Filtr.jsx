...
//     const response = await apiClient.get(`http://127.0.0.1:8000/przedmiot/?nazwa=${nazwa}`);
//     console.log("response: ", response.data);
//     props.parentCallBack(response.data);
//   };

  const MiastoFiltr = async (miasto) => {
    const response = await apiClient.get(`http://127.0.0.1:8000/przedmiot/?miasto=${miasto}`);
    console.log("response:",response.data);
    props.parentCallBack(response.data);
  };
